...
void print() {
    ...
    for (auto job: finish_queue) {
        // 依次计算 到达时间 开始时间 运行时间 完成时间 周转时间 带权周转时间 打印输出 作业名字长度固定为10
        ...
        cout << "作业名字：" << job->name << "\t到达时间：" << arrival_time << "\t开始时间：" << start_time << "\t运行时间："
             << service_time << "\t完成时间：" << end_time << "\t周转时间：" << turnaround_time << "\t带权周转时间："
             << weighted_turnaround_time << endl;
    }
}
...
void join_queue(JobCtrlBlock *job) {
    ...
    job_queue.push(job);
    string text = "装入器：" + job->name + "加入就绪队列";
    cout << text <<endl;
}
...
void init() {
    ...
    vector<JobCtrlBlock *> jobs = {job1, job2, job3, job4, job5, job6, job7, job8, job9, job10};
    for (int i = 0; i < 10; i++) {
